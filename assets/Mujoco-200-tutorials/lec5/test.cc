...
double a3[2] = {0}, a2[2] = {0}, a1[2] = {0}, a0[2] = {0};
...
void generate_trajectory(double t0, double tf, double q_0[2], double q_f[2]) {
	double t0_tf_3 = (t0 - tf)*(t0 - tf)*(t0 - tf);

	for(int i=0; i<2; i++) {
		double q0 = q_0[i], qf = q_f[i];
		a3[i] = -(2*(q0 - qf))/t0_tf_3;
		a2[i] = (3*(q0*t0 + q0*tf - qf*t0 - qf*tf))/t0_tf_3;
		a1[i] = -(6*tf*(q0*t0 - qf*t0))/t0_tf_3;
		a0[i] = -(-qf*t0*t0*t0 + 3*qf*t0*t0*tf -3*q0*t0*tf*tf + q0*tf*tf*tf)/t0_tf_3;
	}
}
